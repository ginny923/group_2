...
class PlayScene(Scene):
    ...
    def update(self, dt: float) -> None:
        ...
        if getattr(self, "apple_sys", None) is not None:
            ...
            # 在這裡補上 self.game.sound
            self.apple_sys.update(dt, [self.p1, self.p2], sound=self.game.sound, avoid_rects=[spawn_left, spawn_right])
        ...
        for b in self.bullets[:]:
            ...
            # (B) 打到爆炸桶
            if barrels and barrels.handle_bullet_hit(b.rect, [self.p1, self.p2], sound=self.game.sound):
                self.bullets.remove(b)
            ...
            if b.owner_id == 1 and b.rect.colliderect(self.p2.body_hitbox()):
                # 傳入 self.game.sound，P2 受傷會播 scream2.mp3
                self.p2.take_damage(b.damage, self.game.sound)
                self.bullets.remove(b)
                continue

            if b.owner_id == 2 and b.rect.colliderect(self.p1.body_hitbox()):
                #======================================
                self.p1.take_damage(b.damage, self.game.sound)
                self.bullets.remove(b)
                continue
    ...
    def _explode(self, g: Grenade) -> None:
        self.game.sound.play("explosion", volume=0.6)
        ...
        if self.barrels:
            self.barrels.explode_at(g.pos, [self.p1, self.p2], sound=self.game.sound)  # 爆炸可以引爆附近桶
