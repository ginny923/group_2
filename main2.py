...
class Player:
    ...
    def take_damage(self, dmg: int , sound: SoundManager = None) -> None:
        if self.hp <= 0: return 
        self.hp = max(0, self.hp - dmg)
    
    # --- 新增：受傷叫聲邏輯 ---
        if sound is not None:
        # P1 會播 "hurt_p1"，P2 會播 "hurt_p2"
            sound_tag = f"hurt_p{self.id}"
            sound.play(sound_tag, volume=1)
...
class Game:
    def __init__(self) -> None:
        ...
        pygame.mixer.music.set_volume(0.4)    # 設定音量 (0.0 到 1.0)
        ...
        # --- 新增這兩行 ---
        self.sound.load("hurt_p1", "scream.mp3")  # P1 使用 scream.mp3
        self.sound.load("hurt_p2", "scream2.mp3") # P2 使用 scream2.mp3
        self.sound.load("apple", "apple.mp3")
        self.sound.load("teleport", "transmit.mp3")
        self.sound.load("explosion", "bomb.mp3")
        ...
        self.play_scene_factory = lambda: __import__("play_scene").PlayScene(self)
