...
class Player:
    ...
    def try_shoot(self, sound: SoundManager) -> List[Bullet]:
        ...
        if bullets:
            ...
            # --- [改寫成這一段] ---
            wpn_name = self.weapon.name
            if hasattr(self, 'game') and wpn_name in self.game.weapon_sounds:
                self.game.weapon_sounds[wpn_name].play()
            else:
                sound.play("shoot", volume=0.25) # 萬一失敗的備用聲音
        return bullets
...
class Game:
    def __init__(self) -> None:
        ...
        # 建立一個音效字典，方便管理
        self.weapon_sounds = {
        "Pistol": pygame.mixer.Sound("pistol_shot.mp3"),
        "Rifle": pygame.mixer.Sound("rifle_shot.mp3"),
        "Shotgun": pygame.mixer.Sound("shotgun_shot.mp3")
        }

        # 調整各自的音量 (0.0 ~ 1.0)
        self.weapon_sounds["Pistol"].set_volume(0.4)
        self.weapon_sounds["Rifle"].set_volume(0.5)
        self.weapon_sounds["Shotgun"].set_volume(0.8)
